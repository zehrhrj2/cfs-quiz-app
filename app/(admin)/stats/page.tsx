...
async function getStats(locale: Locale) {
  const suffix = `:${locale}:quiz_opened`;
  let cursor = 0;
  ...
  for (const src of sources) {
    pipeline.get(`${src}:${locale}:quiz_opened`);
    pipeline.get(`${src}:${locale}:quiz_completed`);
    pipeline.get(`${src}:${locale}:install_clicked`);
    pipeline.get(`${src}:${locale}:score_sum`);
    pipeline.get(`${src}:${locale}:score_count`);
  }
  ...
  return sources
    .map((src, i) => {
      ...
      return {
        ...
        installRate:
          completed > 0 ? Math.round((installed / completed) * 100) : 0,
        avgScore:
          scoreCount > 0
...
export default async function StatsPage({
  ...
  const locale: Locale = localeParam === "uk" ? "uk" : "ru";

  const rows = await getStats(locale);
  ...
  return (
    <div className="min-h-screen bg-bg p-8">
      ...
      <div className="flex gap-4 mb-6 text-sm">
        <a
          href="?locale=ru"
          className={locale === "ru" ? "font-bold underline" : "text-muted"}
        >
          RU
        </a>
        <a
          href="?locale=uk"
          className={locale === "uk" ? "font-bold underline" : "text-muted"}
        >
          UK
        </a>
      </div>
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-muted">
          No data for {locale.toUpperCase()} yet.
        </p>
      ) : (
          ...
          <table className="text-sm border-collapse w-full">
            <thead>
              <tr className="border-b border-border text-left">
                ...
                <th className="py-2 pr-8 font-semibold text-right">
                  Completed
                </th>
                <th className="py-2 pr-8 font-semibold text-right">
                  Installed
